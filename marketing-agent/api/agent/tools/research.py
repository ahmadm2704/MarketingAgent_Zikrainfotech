...
def research_seo_keywords(
    ...
    if serp_key:
        try:
            search = GoogleSearch({
                "q": topic + (" " + industry if industry else "") + " keywords",
                "api_key": serp_key,
                "gl": "us",
                "hl": "en"
            })
            ...
            serp_data = "Here is REAL-TIME Google Search data for this topic. Incorporate this directly into your report:\n"
            if related:
                serp_data += "Top Related Searches (High volume long-tail):\n- " + "\n- ".join([r.get('query', '') for r in related]) + "\n\n"
            if questions:
                serp_data += "People Also Ask (Featured snippet opportunities):\n- " + "\n- ".join([q.get('question', '') for q in questions]) + "\n\n"
            if organic:
                serp_data += "Top Ranking Competitors (Analyze these for content gaps):\n"
                for o in organic:
                    serp_data += f"- {o.get('title')} ({o.get('link')})\n"
        except Exception:
            pass
